...
// MVS picked newer minors that removed symbols still referenced by docker/storage.
// Pin runtime-spec v1.2.x so podman v5.7.x (LinuxPids.Limit int64) matches; use
// moby/buildkit v0.25.x (same line as docker 28.5.x) so the graph stays on v1.2.x.
replace (
