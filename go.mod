...
// docker v28.5.x and fsouza/go-dockerclient expect moby/go-archive v0.1.x root API (Compression, Uncompressed).
// go.podman.io/storage expects filepath-securejoin to re-export OpenInRoot/Reopen at the root (removed in v0.6.0).
// containers/podman v5.7.x assigns int64 to LinuxPids.Limit; runtime-spec v1.3.0 changed that field to *int64.
// runtime-tools must match the same specs-go API as runtime-spec (see runtime-tools generate_linux.go).
// CDI v1.1.x expects runtime-spec v1.3.x (NetDevices, LinuxIntelRdt); pin CDI v1.0.x with runtime-spec v1.2.1.
replace (
	...
	github.com/opencontainers/runtime-tools => github.com/opencontainers/runtime-tools v0.9.1-0.20250523060157-0ea5ed0382a2
	tags.cncf.io/container-device-interface => tags.cncf.io/container-device-interface v1.0.1
	tags.cncf.io/container-device-interface/specs-go => tags.cncf.io/container-device-interface/specs-go v1.0.0
)
